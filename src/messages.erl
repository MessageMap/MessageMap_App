...
push("latest", _, _, _, _) ->
    {422, #{
        status =>
            'Topic Is attached to Schemas Send message with schema: /messages/{SchemaVersion}/{topicName}'
    }};
                    ...
                    if
                        ...
                        true ->
                            ...
                            if
                                SchemaIsValid /= error ->
                                    process_Messages(
                                        TopicId, MapPayload, AppId, SchemaId, RequestTime
                                    );
                                true ->
                                    {422, #{
...
process_Messages(TopicId, MapPayload, AppId, SchemaId, RequestTime) ->
    ...
    R = lists:map(
        fun(App) ->
            {_, FoundAppId, _, _, _, _, SubscribedTopics, _, FilterSettings, EncryptValue,
                PushMessages, PushUrl, PushRetries, PushStatusCode, PushHeaders} = App,
            CaseResult =
                case string:str(SubscribedTopics, TopicId) of
                    ...
                    _ ->
                        ...
                        MapMessageResult =
                            case FilterSettings of
                                <<"[]">> ->
                                    MapPayload;
                                _ ->
                                    JsonFilter = [
                                        processMessageMap(TopicId, X, MapPayload)
                                     || X <- jiffy:decode(FilterSettings)
                                    ],
                                    lists:nth(1, JsonFilter)
                            end,
                        % Start Encryption Values
                        SavedPayload =
                            case EncryptValue of
                                [] ->
                                    MapMessageResult;
                                _ ->
                                    binary:list_to_bin(
                                        encryption:msgEncryption(
                                            jiffy:encode(MapMessageResult),
                                            binary:list_to_bin(EncryptValue)
                                        )
                                    )
                            end,
                        ...
                        PushResult =
                            if
                                PushMessages =:= <<"true">> andalso
                                    SavedPayload =/= "Payload Is To Long" ->
                                    Headers = [
                                        erlang:list_to_tuple(
                                            lists:map(
                                                fun(X) -> lists:delete($;, X) end,
                                                string:tokens(H, ":")
                                            )
                                        )
                                    ...
                                    pushMessages(
                                        erlang:binary_to_list(PushUrl),
                                        erlang:binary_to_integer(PushStatusCode),
                                        Headers,
                                        jiffy:encode(SavedPayload),
                                        erlang:binary_to_integer(PushRetries)
                                    );
                                true ->
                                    true
                            end,
                        % Change This to if HD is Full
                        HardDriveNotFull = true,
                        Result =
                            if
                                HardDriveNotFull andalso SavedPayload =/= "Payload Is To Long" andalso
                                    PushResult ->
                                    database:insert_dyn_table(
                                        FoundAppId, TopicId, SchemaId, SavedPayload, RequestTime
                                    ),
                                    true;
                                true andalso PushResult ->
                                    tools:log(
                                        "info",
                                        io_lib:format(
                                            "Tried to Push Messages to App: ~p Hard Drive is Full",
                                            [AppId]
                                        )
                                    ),
                                    false;
