...
send_data(_) ->
    ...
    Data = #{
        <<"_id">> => erlang:list_to_binary(
            lists:concat(string:replace(erlang:atom_to_list(node()), "@", "-")) ++ "_" ++
                lists:concat(erlang:tuple_to_list(local_ip_v4()))
        ),
        <<"msgmap">> => ws_handler:pullData()
    },
    httpc:request(
        post,
        {?statsUrl, [], "application/json", jiffy:encode(Data)},
...
log(Level = "info", MsgRaw) ->
    ...
    MsgWrite = erlang:binary_to_list(
        erlang:iolist_to_binary(io_lib:format('["~s"] - ~p~n', [Level, Msg]))
    ),
...
verifyAuth(Req) ->
    ...
    #{messageMapAuth := AuthValue} = cowboy_req:match_cookies(
        [{messageMapAuth, [], <<"Bad">>}], Req
    ),
    if
        AuthValue == <<"Bad">> ->
            {AuthValue, Req};
