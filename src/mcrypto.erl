...
-define(charSet,
    " abcdefghijklmnopqrstuvwxyz1234567890QWERTYUIOPASDFGHJKLZXCVBNM,./;[]~'\|`!@#$%^&*(_-=+)"
).
...
encrypt(StringToEncrypt, Salt) ->
    ...
    EncryptedString = runEncoding(
        StringToEncrypt, StartPosition, UpdateSalt, StartFlip, 100, Offset, "", StartFlip
    ),
...
validation(StringToEncrypt, Hash) ->
    ...
    EncryptedString = runEncoding(
        StringToEncrypt, StartPosition, SaltEncode, StartFlip, 100, Offset, "", StartFlip
    ),
    CheckHash =
        "Map" ++ numstr(Steps) ++ "$" ++ numstr(Offset) ++ numstr(StartFlip) ++ "$" ++
            EncryptedString,
    (CheckHash == Hash).
